...
function smartimg_custom_column($column_name, $id, $meta = null)
{
	...
	if ('smartimg' === $column_name) {
		...
		if (is_array($meta)) {
			if (!empty($meta['file']) && false != strpos($meta['file'], 'https://images-na.ssl-images-amazon.com')) {
			...
			if (class_exists('Amazon_S3_And_CloudFront') && preg_match('/^(http|s3|gs)\w*:/', get_attached_file($id))) {
				echo '<div>' . esc_html__('Offloaded Media', 'smartimg') . '</div>';
				return;
			}
			if (class_exists('S3_Uploads') && preg_match('/^(http|s3|gs)\w*:/', get_attached_file($id))) {
				echo '<div>' . esc_html__('Amazon S3 image', 'smartimg') . '</div>';
				return;
			}
			if (class_exists('wpCloud\StatelessMedia') && !empty($meta['gs_link'])) {
