...
if ( ! function_exists( 'imagecreatefrombmp' ) ) {
	...
	function imagecreatefrombmp( $filename ) {
		...
		if ( ! is_readable( $filename ) ) {
			...
			trigger_error( sprintf( __( 'imagecreatefrombmp: Can not open %s!', 'smartimg' ), $filename ), E_USER_WARNING );
			return false;
		}
		...
		if ( 19778 !== (int) $meta['type'] ) {
			/* translators: %s: the image filename */
			trigger_error( sprintf( __( 'imagecreatefrombmp: %s is not a bitmap!', 'smartimg' ), $filename ), E_USER_WARNING );
			return false;
		}
		// read image header.
		...
		if ( $meta['imagesize'] < 1 ) {
			...
			if ( $meta['imagesize'] < 1 ) {
				...
				if ( $meta['imagesize'] < 1 ) {
					/* translators: %s: the image filename */
					trigger_error( sprintf( __( 'imagecreatefrombmp: Cannot obtain filesize of %s !', 'smartimg' ), $filename ), E_USER_WARNING );
					return false;
				}
			}
		}
		...
		$y    = $meta['height'] - 1;
		/* translators: %s: the image filename */
		$error = sprintf( __( 'imagecreatefrombmp: %s has not enough data!', 'smartimg' ), $filename );
			...
			while ( $x < $meta['width'] ) {
				switch ( $meta['bits'] ) {
					...
					default:
						...
						trigger_error( sprintf( __( 'imagecreatefrombmp: %1$s has %2$d bits and this is not supported!', 'smartimg' ), $filename, $meta['bits'] ), E_USER_WARNING );
						return false;
				}
